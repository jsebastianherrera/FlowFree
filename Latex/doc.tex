...
\begin{abstract}
	En este documento se presenta la documentaci\'on correspondiente a la implementaci\'on desarrollada para realizar el videojuego Flow Free.
...
\begin{itemize}
	\item \textbf{Entradas:}
	      \begin{itemize}
		      \item $M^{nxn} ~ | ~ \forall M_{ij} \in \mathbb{Z} ~ \land ~ \ni M_{ij} = M_{i+aj+b}$ 
	      \end{itemize}
	\item \textbf{Salida:}
	      \begin{itemize}
...
\begin{itemize}
    \item El sistema debe dejar en claro al usuario los distintos puntos a conectar mediante distintos colores y un código identificador.
    \item El sistema debe generar diversas posibilidades de mapas aleatoriamente.
    \item El sistema debe permitir la interacción con el usuario por medio de entradas del teclado.
    \item El nivel debe finalizar automaticamente cuando el usuario complete todos los caminos.
\end{itemize}
...
\begin{itemize}
    \item La terminal donde se ejecute el programa debe ocupar como mínimo la mitad de la pantalla.
    \item La terminal debe soportar 256 colores.
    \item Se deben completar todos los puntos para poder completar el nivel.
    \item Los caminos no se pueden intersectar.
    \item Se debe llenar todas las casillas del mapa.
    \item El usuario que ejecute el programa debe tener instalado Python 3 y la librería Curses.
\end{itemize}
\newpage
...
\vspace{1cm}
\begin{figure}[H]
	\centerline{\includegraphics[scale=0.35]{sequence.jpg}}
...
\begin{enumerate}
	...
	\item Asegurese de tener python3 instalado y contar con las siguiente libreria :
		  ...
		  \textbf{Nota: La terminal utilizada para correr el programa debe soportar 256 colores.}
		      ...
		      \item \textbf{play}: Se gener\'a un n\'umero aleatorio para elegir un archivo del 1-22.
		            ...
		            \begin{enumerate}
			            \item[1.] Al moverse de acuerdo a las Keybindings si se para en la posici\'on de alg\'un punto que este pintado de un color diferente al blanco y da ``enter'' el color se selecciona.
				            \begin{figure}[H]
					            \centerline{\includegraphics[scale=0.25]{chosen.jpg}}
			            ...
			            \item[2.] Una v\'ez elegido el color en la esquina superior izquierda de la pantalla de acuerdo a la \textbf{Figura 4} se mostrar\'a el color seleccionado.\\
				            \begin{enumerate}
					            ...
					            \item[2.2] Para completar un ``flow'', el usuario debe moverse de acuerdo a las Keybindings y conectar el ``flow'' con el punto destino, una v\'ez lo complete el programa inmediatamente deselecciona el color previamente elegido.
					              \item[2.3] En caso de querer deseleccionar un color se debe oprimir  ``enter'' en un cuadro que no se un punto de conexi\'on a un flow.
				            \end{enumerate}
